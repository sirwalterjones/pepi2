...
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow, TableCaption } from "@/components/ui/table"; // Import Table components
...
export default function MonthlyUnitReport() {
    ...
    const [reportData, setReportData] = useState<MonthlyUnitReportData | null>(null);
    ...
    const renderTotals = (totals: MonthlyUnitReportTotals) => (
        <div className="mb-6 p-4 border rounded-lg bg-muted/40">
            ...
            <dl className="grid grid-cols-2 gap-x-4 gap-y-2 text-sm">
                ...
                <dd className="text-right">{formatCurrency(totals.totalAgentIssues)}</dd>
                <dt className="font-medium">Agent Returns:</dt>
                <dd className="text-right">{formatCurrency(totals.totalAgentReturns)}</dd>
                <dt className="font-medium">Expenditures:</dt>
                <dd className="text-right">{formatCurrency(totals.totalExpenditures)}</dd>
                <dt className="font-medium">Additional Funds Added:</dt>
                <dd className="text-right">{formatCurrency(totals.totalAdditionalUnitIssue)}</dd>
            </dl>
        </div>
    );

    const renderTransactionTable = (transactions: MonthlyUnitReportTransaction[]) => (
                ...
                <TableBody>
                    {transactions.length === 0 ? (
                        ...
                        transactions.map((tx) => {
                            ...
                            return (
                                <TableRow key={tx.id}>
                                    <TableCell className="whitespace-nowrap">{format(new Date(tx.created_at), "P p")}</TableCell>
                                    <TableCell><Badge variant={typeVariant} className="capitalize">{tx.transaction_type?.replace('_', ' ')}</Badge></TableCell>
                                    <TableCell className="text-right">{formatCurrency(tx.amount)}</TableCell>
                                    <TableCell>{tx.description}</TableCell>
                                    <TableCell>{tx.agent_name || 'N/A'}</TableCell>
                                    <TableCell><Badge variant={statusVariant} className="capitalize">{tx.status}</Badge></TableCell>
